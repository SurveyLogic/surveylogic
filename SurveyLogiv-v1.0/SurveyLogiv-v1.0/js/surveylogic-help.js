...
function HelpSystem(helptext_span) {
  ...
  // Initial text.
  printHelp(
    "(Only your browser reads the image. It is not uploaded anywhere.)"
  );

  this.achievementUnlocked = function (level) {
    if (level < this.last_help_level_) return;
  ...
  function printLevel(requested_level) {
    switch (requested_level) {
      ...
      case HelpLevelEnum.DONE_FINISH_LINE:
        printHelp(
          "TIP: starting a line where another ends measures their angles."
        );
        break;
