...
//variable for exporting to svg
window.loupe_canvas_size_forSVG = [];

// Init function. Call once on page-load.
function surveylogic_init() {
  help_system = new HelpSystem(document.getElementById("helptext"));
  aug_view = new surveylogicView(document.getElementById("measure"));
...
function surveylogicController(canvas, view) {
...
function init_download(filename) {
  ...
  var download_link = document.getElementById("download-result");
  download_link.download = "surveylogic-" + filename + ".png";
...
function load_background_image(chooser) {
  ...
  img_reader.onload = function (e) {
    ...
    new_img.onload = function () {
      ...
      var canvasDataFormat = {
        x: new_img.width,
        y: new_img.height,
    };

      loupe_canvas_size_forSVG = canvasDataFormat;
      
      var var_loupesize = JSON.stringify(loupe_canvas_size_forSVG);
      console.log("var_loupesize", var_loupesize);
      

      let newLoupeSizeEvent = new CustomEvent('newLoupeSizeAdded', { detail: var_loupesize });
      window.dispatchEvent(newLoupeSizeEvent);
    ...
    // Save new.img to local storage
    localStorage.setItem("UserImage", new_img.src);
  };
}
