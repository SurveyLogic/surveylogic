...
function surveylogicView(canvas) {
  "use strict";
  ...
  this.resetWithSize = function (width, height) {
    ...
    // A fresh model.
    this.model_ = new surveylogicModel();
    if (this.controller_ == undefined) {
      this.controller_ = new surveylogicController(canvas, this);
    }
  };

  this.getUnitsPerPixel = function () {
    return this.print_factor_;
  };
  this.setUnitsPerPixel = function (factor) {
    this.print_factor_ = factor;
  };
  this.setShowDeltas = function (b) {
    this.show_deltas_ = b;
  };
  this.setShowAngles = function (b) {
    this.show_angles_ = b;
  };
  ...
  this.getCanvas = function () {
    return this.measure_canvas_;
  };

  // Draw all the lines!
  this.drawAll = function () {
    this.measure_ctx_.clearRect(
      0,
      0,
      this.measure_canvas_.width,
  ...
  function drawArc(ctx, arc, radius_fiddle) {
    ...
    ctx.arc(
      arc.center.x,
      arc.center.y,
      Math.min(radius, arc.max_radius),
      2 * Math.PI - arc.end,
      2 * Math.PI - arc.start
    );
    ctx.stroke();
  ...
  function drawMeasureLine(ctx, line, length_factor, show_deltas, highlight) {
      ...
      drawT(ctx, line.p1.x, line.p1.y, line.p2.x, line.p2.y, end_bracket_len);
      drawT(ctx, line.p2.x, line.p2.y, line.p1.x, line.p1.y, end_bracket_len);
      ctx.stroke();
  ...
  function drawDeltaLine(ctx, line, length_factor, highlight) {
    ...
    if (Math.abs(dy) > non_overlap_target) {
      var line_y =
        line.p1.y + dy - (dy < 0 ? -non_overlap_target : non_overlap_target);
